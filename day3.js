...
given
  set of directions
  housesVisited starts at 1 by default
  starting location of [0,0]
find
  number of housesVisited

santa moves
if he's in a new location
  increment housesVisited

track his location hist in an object
  where keys are latxlong, vals are 1 if interested
*/

var houseCount = function(input){
  var len = input.length;
  var location = [0,0]; // lat, long
  var housesVisited = 1;
  var hist = {'0x0': 1};

  var historyCheck = function(loc){
    var locStr = loc[0] + 'x' + loc[1];
    if(!hist[locStr]){
      housesVisited += 1;
      hist[loc[0] + 'x' + loc[1]] = 1;
    }
  };
